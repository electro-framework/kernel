...
class ModulesInstaller
{
	...
	function __construct(KernelSettings $kernelSettings, ConsoleApplication $consoleApp, ModulesRegistry $modulesRegistry, callable $migrationsAPIFactory, ProfileInterface $profile)
	{
		...
		if ($migrationsAPI)
		{
			...
			if ($migrations)
			{
				...
				if ($migrations)
				{
					...
					$io->comment("    No reverse migrations were run.")->nl();
			}
		}
	}
	...
	{
		return map($modules, function (ModuleInfo $module)
		{
			return $module->name;
		});
	}
	...
	static private function modulesTopologicalSort(array $modules)
	{
		...
		/** @var ModuleInfo[] $All All modules, indexed by name */
		$All = [];
		/** @var ModuleInfo[] $Sorted The sorted list to be returned */
		$Sorted = [];
		foreach ($modules as $module)
		{
			$All[$module->name] = (object) [
					'module' => $module,
					'requiredBy' => $module->requiredBy, // tmp is a temporary scratch list
			];
		}

		/** @var ModuleInfo[] $S Set of all modules not dependend upon */
		$Root = filter($modules, function (ModuleInfo $m)
		{
			return !$m->requiredBy;
		}, true);
		while ($Root)
		{
			$current = array_pop($Root);
			array_unshift($Sorted, $current);
			foreach ($current->dependencies as $name)
			{
				$node = get($All, $name);
				if ($node && in_array($current->name, $node->requiredBy))
				{
					$node->requiredBy = array_diff($node->requiredBy, [$current->name]);
					if (!$node->requiredBy)
						$Root[] = $node->module;
				}
			}
		}
		foreach ($All as $name => $node)
		{
			if ($node->requiredBy)
				throw new ConfigException(sprintf('Cyclic dependency between modules: "%s" <-> "%s"', $node->module->name, implode('" <-> "', $node->requiredBy)));
		}
...
//  }
	...
	private function getMigrationsAPI()
	{
		try
		{
			return $this->migrationsAPI ?: (($factory = $this->migrationsAPIFactory) ? $this->migrationsAPI = $factory() : null);
		}
		...
		foreach ($modules as $module)
		{
			foreach ($module->dependencies as $dep)
				if (isset($all[$dep]))
					...
					throw new ConfigException("Invalid dependency: $dep");
		}
	...
	private function updateMigrationsOf($moduleName)
	{
		...
		if ($migrations)
		{
			...
			if ($migrations)
			{
				...
				$io->comment("No new migrations to run")->nl();
			$io->indent(0);
		}
	}
